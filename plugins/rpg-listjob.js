const {MessageType, WAPresence, MessageOptions} = require('@adiwajshing/baileys');
let handler = async (m, { conn, text, participants }) => {
  const format = num => {
	...
	conn.reply(m.chat, `*[ LIST JOB ]*\n\n${user.map(v => '  ○ @' + v.replace(/@.+/, '') + ' [ ' + global.DATABASE.data.users[v].job + ' - Rp. ' + format(global.DATABASE.data.users[v].exp)  +' ]').join('\n')}\n\nKetik .sewajob @user untuk menyewa orang`, m,{ contextInfo: { mentionedJid: user } })
}
